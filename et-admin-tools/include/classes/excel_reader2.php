...
//  return(array_comb($k,split(":",gmdate('s:i:G:j:w:n:Y:z:l:F:U',is_null($ts)?time():$ts)))); // split deprecated
...
class OLERead {
  ...
  /*
   * read method changed to accept a string representation of the file
   * rather than the file itself. This was done to allow a string representation 
   * of the file as an optional paparameter to the Spreadsheet_Excel_Reader class
   * constructor.
   *
   * @author Bruce LeCaptain <brulecap@europeantableware.com>
   *
   * @param string $xls_data_string a string representation of the file.
   *
   */
  function read($xls_data_string){
...
class Spreadsheet_Excel_Reader {
  ...
  function sheetcount($limit=25) {
    $num_sheets = 0;
    while (true) {
      if ((isset($this->sheets[$num_sheets])) && ($num_sheets < $limit)) {
      $num_sheets += 1;
      } else {
        break;
      }
...
//    $parts = split(";",$format); //Split deprecated
   ...
   *
   * 31 May, 2016 - Changed to accept a string representation of the file. Also
   * made change to the way errors were being handled. The previous version was
   * dying if the file was not readable. I would rather be given the opportunity 
   * to handle the error rather than dying.
   *
   * @author Bruce LeCaptain <brulecap@europeantableware.com>
   *
   * @param string $file file name.
   * @param bool $store_extended_info if true extended info will be stored.
   * @param string $outputEncoding output encoding
   * @param string $xls_data_string a string representation of the file.
   *
   */
  function Spreadsheet_Excel_Reader($file='',$store_extended_info=true,$outputEncoding='', $excel_string='') {
    ...
    if ($excel_string != '') {
      $this->read($excel_string);
    } else if ($file!='') {
      if(!is_readable($file)) {
        $this->is_readable = false;
      } else {
        $xls_string_from_file = @file_get_contents($file);
        if ($this->read($xls_string_from_file) === false) $this->is_readable = false;
      }
    } else {
      $this->is_readable = false;
    }
